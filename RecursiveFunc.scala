...
object RecursiveFunc {
  ...
  */
  /*Ответ: возможно только в том случае, если дерево одностороннее. То есть идет либо только в
  порядке убывания, либо только в порядке возрастания.  И возможно провести какие-нибудь итерации.
  Иначе в хвостовой рекурсии не получится перескакивать по веткам. Ветка должна быть одна.
   */
}
